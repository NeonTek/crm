...
interface LoginFormProps {
  ...
  onSuccess?: () => void; // callback to close modal/drawer on success
}
...
export function LoginForm({ onSwitchToRegister, onSuccess }: LoginFormProps) {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (onSuccess) onSuccess(); // ⬅️ close modal/drawer if provided
    } catch (error: any) {
      // Error is already handled by auth-context (toast, etc.)
    } finally {
      setLoading(false);
    }
  };
